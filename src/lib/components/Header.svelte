...
<header class="header bg-white shadow py-4 px-4">
	<div class="header-content flex items-center flex-row">
		...
		<div class="flex ml-auto items-center">
			<div class="flex flex-row items-center justify-end me-2 pe-4">
				<img src="{portrait}" alt class="h-8 w-8 bg-gray-200 rounded-full" />
			...
			<div class="relative inline-block text-left">
				<button type="button" on:click={toggleProfileDropdown} class="inline-flex w-8 h-8 items-center justify-center" id="profile-button" aria-expanded="false" aria-haspopup="true">
				  <span class="sr-only">EUI profile: {user_data.ict.Firstnames} {user_data.ict.Lastnames}</span>
				  <span class="fa-regular fa-angle-down fa-lg {isProfileDropdownOpen ? 'fa-rotate-180' : ''}" aria-hidden="true"></span>
				</button>
