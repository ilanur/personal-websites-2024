				...
				searchBox({
					...
					templates: {
						submit({ cssClasses }, { html }) {
							return html`<span class="fa-solid fa-fw fa-magnifying-glass"></span>`;
						},
					},					
				}),
				hits({
						...
						empty(results, { html }) {
							return html`
								<div class="${not_found_classes}">
									...
									<h3>No results found for "<strong>${results.query}</strong>"</h3>
				...
				voiceSearch({
					...
					templates: {
						buttonText({ isListening, status, errorCode, transcript, isSpeechFinal, isBrowserSupported }, { html }) {
							return html`<span class="fa-regular fa-fw ${isListening ? 'fa-regular' : 'fa-solid'} fa-microphone"></span>`;
						},
					},
				}),
				hitsPerPage({
